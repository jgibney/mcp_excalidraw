...
- 2026-07-22 (same session, at Jim's request): killed the stale server process (`kill` on the PID running since 2026-07-21). Claude Code's own MCP config (`~/.claude.json`) already pointed at the correct current path (`/home/jim/src/external_sources/mcp_excalidraw/dist/index.js`), so a fresh launch should pick it up correctly — but **Claude Code does not auto-relaunch a killed MCP server mid-session**; confirmed the `mcp__excalidraw__*` tools went fully unavailable immediately after the kill, not a brief blip. Jim needs to run `/mcp` to reconnect (or it'll pick up fresh on the next new session). Once reconnected, the stale-export-path issue documented above should be resolved — worth verifying that specifically (try `export_scene`/`export_to_image` against the new `~/src/excalidraw_examples/...` path) next time this server is used, to confirm the fix actually took rather than assuming it did.
